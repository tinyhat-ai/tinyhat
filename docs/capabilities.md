...
| `tinyhat_services` | Platform API required | Discovers Stripe Projects services and lets the Computer's agent provision and manage free or verifiably fixed-price services within the owner's allowance after one account-wide authorization. Each write has a durable operation ID; provider-specific sign-in or facts may still need the owner. |
