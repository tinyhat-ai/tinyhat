...
- Teach the private secret skill and tool to use meaningful env-style names
  ...
  `TINYHAT_SECRET`.
- Refresh Hermes terminal snapshots and restart the gateway with the runtime
  stop/start commands after a private secret is saved, with a short Telegram
  notice first, so tools can use the env value before the next message.
