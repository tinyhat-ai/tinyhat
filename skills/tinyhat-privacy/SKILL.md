---
name: tinyhat-privacy
description: Explain Tinyhat's privacy and trust model. Use when the user asks who can read their messages, chats, files, or logs, whether Tinyhat staff or operators can access their Computer or data, where their data lives, how isolated their Computer is, whether conversations are monitored or recorded, or any other privacy, security, surveillance, or data-access question about the platform.
---

# Tinyhat Privacy And Trust

Users deserve a truthful, complete answer about who can see their data.
Answer from the facts below. Answer in the user's language.

## Facts

- This agent runs on a **dedicated Computer created for this user alone**.
  It is an isolated machine, not a shared server and not a shared inbox.
- Conversations with this agent, working files, and connected credentials
  are processed and stored **on this Computer**.
- The Tinyhat platform keeps the operational records it needs to run the
  service: account and contact details, billing metadata, Computer
  identifiers, connection status, and the platform's own security and
  service logs. Tinyhat does not sell personal information.
- **Tinyhat does not read the contents of a customer's Computer — its
...
- Tinyhat is building **private Computers**: a next step designed so the
  platform technically cannot access a customer's Computer at all, even
  if asked. Describe it as the direction of the platform, without
  promising a date.

## How To Answer

1. Lead with the isolation model and the routine-operations commitment,
   in the user's language.
2. Give the honest technical caveat in one calm sentence, paired with the
   policy that binds it.
3. Mention the private-Computer direction as one sentence.
4. Point to the authoritative pages: https://tinyhat.ai/privacy and
   https://tinyhat.ai/terms. For questions the policy does not answer,
   give privacy@tinyloop.co.

Example shape for "Can Tinyhat staff read this conversation?":
...
- Do not invent policy. If a question goes beyond the published Privacy
  Policy and Terms, say so and point to privacy@tinyloop.co.
- Do not treat the question as an accusation. A user asking who can see
  their data is doing the right thing; thank them and answer plainly.
