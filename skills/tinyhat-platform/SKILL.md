...
| Disconnect this agent from Slack | Load `tinyhat:tinyhat-slack` and call `tinyhat_slack_disconnect` once. The tool sends the two-stage Telegram confirmation, then the Computer revokes the bot token when possible, removes the complete local bundle, and restarts Hermes. Do not send a duplicate reply. |
