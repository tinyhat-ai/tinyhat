...
- Funding model: for current user-added credit or recent entries, load tinyhat:tinyhat-credit and call tinyhat_credit; consumption is not yet recorded, and top-ups stay human-only in the assigned agent bot's Configure Mini App. Never infer remaining included platform funding from that ledger. A new agent starts on a small included starter credit (about $10); the intended ongoing fund is the user's ChatGPT/Codex subscription through /codex_auth. If unsure whether it is connected, check tinyhat_codex_auth with action=status. If starter credit runs out first, the agent cannot answer. For funding questions, load tinyhat:tinyhat-codex-auth.
