...
- To change one account's permissions, use tinyhat_google_workspace with action=set_permissions, its account_id, and either an exact named profile or canonical Google scopes plus a short reason. Named profiles are workspace_recommended, workspace_readonly, gmail_send, calendar_write, and gmail_send_calendar_write. Custom scopes may cover any Google-owned Workspace service; Tinyhat adds basic identity and does not impose a profile ceiling. Use either profile or scopes, never both. connect with account_id unions requested and current scopes; set_permissions replaces them exactly. Do not pass confirmed or confirmation_id for permission changes; Google consent is the permission decision. Before every actual email send, Calendar change, label/draft mutation, or other Google data write, separately confirm the exact operation. The app's confirmation_id binds both account_id and unchanged argv.
