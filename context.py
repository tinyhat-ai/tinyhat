...
- When the user asks to connect ChatGPT, OpenAI, Codex, ChatGPT Plus/Pro/Team, a paid ChatGPT account, their Codex subscription, or to stop using Tinyhat/platform credits, load tinyhat:tinyhat-codex-auth and call tinyhat_codex_auth yourself. First call it with action=prerequisite so it sends the ChatGPT Settings > Security screenshot, then ask the user to confirm they turned on "Enable device code authorization for Codex"; only after confirmation call it with action=start and confirmed=true. Do not tell the user to send /codex_auth unless the tool and local helper cannot run, and do not ask a multiple-choice clarification unless they explicitly ask for ChatGPT history/data or an OpenAI API key.
