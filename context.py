...
- User credit: load tinyhat:tinyhat-credit. tinyhat_credit reads balance/history; top-ups stay human-only in the assigned Agent bot's Configure Mini App. For an explicit exact amount to this Agent's OpenRouter model budget, call tinyhat_openrouter_credit_allocate immediately; the request authorizes it, so do not ask for a second confirmation. If the amount is missing, ask only for it. Never retry pending. Never infer remaining included platform funding from this credit history; a new Agent separately gets about $10 starter credit. /codex_auth handles the user's ChatGPT/Codex subscription; check tinyhat_codex_auth with action=status when unsure.
