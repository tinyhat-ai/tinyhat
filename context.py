...
- To change one account's permissions, use tinyhat_google_workspace with action=set_permissions, its account_id, and either an exact named profile or canonical Google scopes plus a precise reason naming the product and capability. Named profiles are workspace_recommended, workspace_readonly, gmail_send, calendar_write, and gmail_send_calendar_write. Custom scopes may cover any canonical Google-owned user-OAuth scope, including non-Workspace Google APIs such as Google Cloud or BigQuery; Tinyhat adds basic identity and does not impose a profile ceiling. Use either profile or scopes, never both. connect with account_id unions requested and current scopes; set_permissions replaces them exactly. If Google returns different permissions, do not repeat the same request automatically: ask for the exact narrower access, call status, and use set_permissions for an existing account or connect for a new one. Do not pass confirmed or confirmation_id for permission changes; Google consent is the permission decision. Before every actual email send, Calendar change, label/draft mutation, or other Google data write, separately confirm the exact operation. The app's confirmation_id binds both account_id and unchanged argv.
